...
# 99 bottle of beer on the wall song
class Bottles
  def verse(bottles)
    <<~VERSE
  ...
  def line_two(bottles)
    if bottles.positive?
      "Take #{pluralize_one(bottles)} down and pass it around, "\
      "#{remaining_bottles(bottles)} "\
      "#{pluralize_bottles(remaining_bottles(bottles))} of beer on the wall."
    else
  ...
  def no_bottles(bottles, sentense_case = false)
    bottles.positive? ? bottles : no_more(sentense_case)
  end

  def no_more(sentense_case)
    sentense_case ? 'No more' : 'no more'
  end
